...
app.post('/products', validateProduct, (req, res) => {
  ...
  if (!errors.isEmpty()) {
    return res.render('products/create', {
      ...
      allErrors: errors.array().map(e => e.msg),
      formData: req.body
    });
  }
...
app.put('/products/:id', validateProduct, (req, res) => {
  ...
  if (!errors.isEmpty()) {
    return res.render('products/edit', {
      title: `Editar: ${products[index].name} - PC Componentes`,
      product: products[index], categories, activeNav: '',
      errorMsg: errors.array()[0].msg,
      allErrors: errors.array().map(e => e.msg),
      formData: req.body
    });
...
app.post('/usuarios/login', validateLogin, (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.render('users/login', {
      title: 'Iniciar sesión - PC Componentes', activeNav: '',
      errorMsg: errors.array()[0].msg,
      allErrors: errors.array().map(e => e.msg),
      formData: req.body
    });
  ...
  if (!user || user.password !== req.body.password) {
    return res.render('users/login', {
      title: 'Iniciar sesión - PC Componentes', activeNav: '',
      errorMsg: 'La contraseña es incorrecta.', formData: req.body
    });
  }
  res.redirect('/');
});
...
app.post('/usuarios/registro', validateRegister, (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.render('users/register', {
      title: 'Crear cuenta - PC Componentes', activeNav: '',
      errorMsg: errors.array()[0].msg,
      allErrors: errors.array().map(e => e.msg),
      formData: req.body
    });
